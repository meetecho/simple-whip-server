...
function setupRest(app) {
	...
	router.post('/endpoint/:id', async function(req, res) {
		...
		try {
			...
			endpoint.handle = await janus.attach(VideoRoomPlugin);
	...
	router.patch('/resource/:rid', async function(req, res) {
	...
	router.delete('/resource/:rid', async function(req, res) {
		let rid = req.params.rid;
		let id = resources[rid];
		if(!rid || !id) {
			res.status(404);
